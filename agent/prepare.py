...
def upload_artifact():
    """Upload ~/.cache/autoresearch/ to W&B as a versioned dataset artifact."""
    ...
    artifact = wandb.Artifact(
        name=ARTIFACT_NAME,
        type="dataset",
        description="Autoresearch training data shards and tokenizer",
    )
    artifact.add_dir(CACHE_DIR)
    run.log_artifact(artifact)
    run.finish()
    print(f"Artifact uploaded: {entity}/{project}/{ARTIFACT_NAME}:latest")


def ensure_cache():
    """Download the dataset artifact if the local cache is absent or incomplete."""
    has_tokenizer = os.path.exists(os.path.join(TOKENIZER_DIR, "tokenizer.pkl"))
    ...
    print(f"Cache missing. Downloading {ARTIFACT_NAME} artifact from W&B...")
    api = wandb.Api()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--upload", action="store_true", help="Upload prepared cache to W&B as a dataset artifact after preparation")
